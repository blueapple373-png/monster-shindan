...
const monsters = {
  fuander:  { name: "フアンダー",  sub: "不安モンスター",    color: "#9B8EC4", bg: "#F0EDFB", img: "public/FUAN.png", imgTreat: "public/FUAN-Treat.png", fuki: "public/FUAN-Fuki.png", num: "01.png", desc: "まだ起きていない未来を心配するモンスター。あなたの見張り番として働いているけれど、時々暴走して、今ここにいることを難しくさせます。", treat: "「今この瞬間」に意識を戻す練習が効きます。深呼吸、五感への集中、「今日起きたいいこと1つ」を見つける習慣。" },
  kako:     { name: "カコノキズ",  sub: "過去の傷モンスター", color: "#E07070", bg: "#FDF0F0", img: "public/KAKO.png", imgTreat: "public/KAKO-Treat.png", fuki: "public/KAKO-Fuki.png", num: "02.png", desc: "過去の痛みを忘れないモンスター。二度と傷つかないよう守ろうとするけれど、過去と今を混同させてしまうことがあります。", treat: "今の状況と過去の状況を「分けて見る」練習が効きます。「これは今のこと？昔のこと？」と自分に問いかける習慣。" },
  jiko:     { name: "ジコヒテイ",  sub: "自己否定モンスター", color: "#6BAE8E", bg: "#EDF7F2", img: "public/HITEI.png", imgTreat: "public/HITEI-Treat.png", fuki: "public/HITEI-Fuki.png", num: "03.png", desc: "自分の価値を否定するモンスター。完璧でなければと追い込んでくる。でも実は、あなたに高い基準を持ってほしいという願いから生まれています。", treat: "「できたこと」を記録する習慣が効きます。小さいことでいい。歯磨きできた、それだけで充分。" },
  jisekin:  { name: "ジセキン",    sub: "自責モンスター",    color: "#6BAEC4", bg: "#EDF4F7", img: "public/JISEKI.png", imgTreat: "public/JISEKI-Treat.png", fuki: "public/JISEKI-Fuki.png", num: "04.png", desc: "全部自分のせいにするモンスター。責任感の強さから生まれているけれど、本来あなたのせいではないことまで背負わせてしまいます。", treat: "「これは自分の責任？相手の責任？」と境界線を引く練習が効きます。責任の範囲を小さく限定する習慣。" },
};
...
function DiamondChart({ scores, maxScore }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      ...
      {ORDER.map((key,i)=>(
        <text key={key} x={labelPos[i].x} y={labelPos[i].y} textAnchor={labelPos[i].anchor}
          fontSize={10} fontWeight="700" fill={monsters[key].color} fontFamily="sans-serif">
          ...
          <div style={{ textAlign:"center" }}>
            ...
            <div style={{ display:"flex", justifyContent:"center", gap:12, marginBottom:32, flexWrap:"wrap" }}>
              {Object.values(monsters).map(m=>(
                <div key={m.name} style={{ background:m.bg, border:`2px solid ${m.color}30`, borderRadius:16, padding:"12px 16px", textAlign:"center", minWidth:90 }}>
                 <img src={m.img} alt={m.name} style={{width:56, height:56, objectFit:"contain"}} />
                ...
                return (
                  <div key={key}>
                    <div style={{ background:m.bg, border:`2px solid ${m.color}30`, borderRadius:20, padding:24, marginBottom:16 }}>
                      <div style={{ fontSize:13, fontWeight:700, color:m.color, marginBottom:8 }}>{m.emoji} {m.name}</div>
